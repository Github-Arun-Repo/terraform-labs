// CI pipeline for the Document Processor service.
//
...
pipeline {
  ...
  parameters {
    ...
    string(
      ...
      defaultValue: '',
      description: 'Full ECR URI, e.g. 123456789012.dkr.ecr.eu-west-1.amazonaws.com/document-processor.'
    )
    string(
      name: 'IMAGE_TAG',
      defaultValue: '',
    ...
    string(
      ...
      defaultValue: 'k8s/eks/document-processor/values.yaml',
      description: 'Repo-relative path to the Helm values file where image.tag is updated.'
    )
  }

  environment {
    APP_PATH = 'applications/document-processor'
  }
        ...
        container('maven') {
          withCredentials([usernamePassword(
            ...
            sh '''
              ...
              git commit -m "ci: update document-processor image tag to ${RESOLVED_IMAGE_TAG} [skip ci]"
