pipeline {
  ...
  parameters {
    ...
    string(name: 'ECR_REPOSITORY_URI', defaultValue: '', description: 'Full ECR URI for user-management-service image.')
    ...
    string(name: 'VALUES_FILE_PATH', defaultValue: 'k8s/eks/user-management-service/values.yaml', description: 'Helm values file to patch.')
  }

  environment {
    APP_PATH = 'applications/user-management-service'
  }
        ...
        container('maven') {
          withCredentials([usernamePassword(
            ...
            sh '''
              ...
              git commit -m "ci: update user-management-service image tag to ${RESOLVED_IMAGE_TAG} [skip ci]"
