pipeline {
  ...
  parameters {
    ...
    string(name: 'ECR_REPOSITORY_URI', defaultValue: '', description: 'Full ECR URI for document-review-service image.')
    ...
    string(name: 'VALUES_FILE_PATH', defaultValue: 'k8s/eks/document-review-service/values.yaml', description: 'Helm values file to patch.')
  }

  environment {
    APP_PATH = 'applications/document-review-service'
  }
        ...
        container('maven') {
          withCredentials([usernamePassword(
            ...
            sh '''
              ...
              git commit -m "ci: update document-review-service image tag to ${RESOLVED_IMAGE_TAG} [skip ci]"
