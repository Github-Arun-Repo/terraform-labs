pipeline {
  ...
  parameters {
    ...
    string(name: 'ECR_REPOSITORY_URI', defaultValue: '', description: 'Full ECR URI for document-processing-service image.')
    ...
    string(name: 'VALUES_FILE_PATH', defaultValue: 'k8s/eks/document-processing-service/values.yaml', description: 'Helm values file to patch.')
  }

  environment {
    APP_PATH = 'applications/document-processing-service'
  }
        ...
        container('maven') {
          withCredentials([usernamePassword(
            ...
            sh '''
              ...
              git commit -m "ci: update document-processing-service image tag to ${RESOLVED_IMAGE_TAG} [skip ci]"
